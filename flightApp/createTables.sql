...
CREATE TABLE Users_ramonrcp(
    username VARCHAR(20) PRIMARY KEY,
    password VARBINARY(256),
    balance INT,
);

CREATE TABLE Reservations_ramonrcp(
    rid INT PRIMARY KEY,
    is_paid BIT, 
    flight_1 INT FOREIGN KEY REFERENCES FLIGHTS(fid),
    flight_2 INT FOREIGN KEY REFERENCES FLIGHTS(fid),
    username VARCHAR(20) NOT NULL FOREIGN KEY REFERENCES Users_ramonrcp(username)
);